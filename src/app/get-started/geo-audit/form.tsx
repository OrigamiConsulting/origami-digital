    ...
    try {
      const response = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          ...
          service: 'geo-audit',
          budget: '',
          message: `GEO Audit Request (AI Search Visibility)\n\nWebsite: ${formData.website}\n\nBiggest search visibility challenge:\n${formData.challenge || '(not provided)'}`,
          fax,
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-xl bg-[#141414] px-6 py-12 text-center">
        ...
        <h3 className="mb-2 font-[family-name:var(--font-display)] text-xl font-bold text-white">
          GEO audit request received!
        </h3>
        <p className="text-sm text-[#8A8A8A]">
          We&apos;ll check how your business appears in AI search and send you a report within 3–5 business days.
        </p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      <div aria-hidden="true" style={{ position: 'absolute', left: '-9999px', width: '1px', height: '1px', overflow: 'hidden' }}>
        <label htmlFor="geo-fax">Fax number (leave blank)</label>
        <input type="text" id="geo-fax" name="fax" tabIndex={-1} autoComplete="off" value={fax} onChange={(e) => setFax(e.target.value)} />
      </div>
      {status === 'error' && (
      ...
      <div>
        <label htmlFor="geo-name" className={labelClasses}>
      ...
      <div>
        <label htmlFor="geo-email" className={labelClasses}>
      ...
      <div>
        <label htmlFor="geo-website" className={labelClasses}>
      ...
      <div>
        <label htmlFor="geo-challenge" className={labelClasses}>
          What&apos;s your biggest search visibility challenge?
        </label>
        <textarea
          id="geo-challenge"
          name="challenge"
          ...
          onChange={handleChange}
          placeholder="e.g. don't appear in Google AI Overviews, competitors are getting cited, not sure where to start…"
