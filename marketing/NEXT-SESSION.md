...
- **LinkedIn Company Page — DONE.** Page live at https://www.linkedin.com/company/origami-digital-za (ID `113156320`). Name, URL slug, industry (IT Services & IT Consulting), size (2–10), type (Privately held), tagline and full 3-pillar About description are all saved. Still TO DO manually (~2 min each): (a) upload logo — `public/images/logo/origami-horse.png` — admin → Edit Page → Logo (file_upload is blocked from the Chrome extension for LinkedIn, so Tinashe must do this); (b) upload cover image — suggest a 1128×191 crop of the origami horse on dark brand background; (c) publish first post (grab the thought-leadership post from `marketing/linkedin-posts-week-1.md`).
